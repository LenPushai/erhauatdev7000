...
public class RfqController {
    ...
    public ResponseEntity<RFQ> partialUpdateRfq(@PathVariable Long id, @RequestBody Map<String, Object> updates) {
        ...
        try {
            ...
            // Add Job tracking
            if (updates.containsKey("jobId")) rfq.setJobId(Long.valueOf(updates.get("jobId").toString()));
            if (updates.containsKey("jobNumber")) rfq.setJobNumber((String) updates.get("jobNumber"));

            // Add Invoice value tracking
            if (updates.containsKey("invoiceValue")) {
                rfq.setInvoiceValue(new BigDecimal(updates.get("invoiceValue").toString()));
            }

            // Add Payment tracking
            if (updates.containsKey("paymentDate")) {
                rfq.setPaymentDate(LocalDate.parse((String) updates.get("paymentDate")));
            }
            if (updates.containsKey("amountPaid")) {
                rfq.setAmountPaid(new BigDecimal(updates.get("amountPaid").toString()));
            }
            if (updates.containsKey("paymentReference")) {
                rfq.setPaymentReference((String) updates.get("paymentReference"));
            }
