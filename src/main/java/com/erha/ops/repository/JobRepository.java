...
public interface JobRepository extends JpaRepository<Job, Long> {
    ...
    // ========================================
    // PARENT/CHILD JOB QUERIES
    // ========================================
    
    /**
     * Find all child jobs for a given parent job ID
     */
    List<Job> findByParentJobId(Long parentJobId);
    
    /**
     * Find all parent jobs (jobs that have children)
     */
    List<Job> findByIsParentJobTrue();
    
    /**
     * Find jobs by parent job ID ordered by sequence
     */
    @Query("SELECT j FROM Job j WHERE j.parentJobId = :parentJobId ORDER BY j.jobSequence ASC")
    List<Job> findChildJobsOrderedBySequence(@Param("parentJobId") Long parentJobId);
}